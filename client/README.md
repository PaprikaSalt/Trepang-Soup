...
- 中途加入：按服务端快照阶段直接进入正在进行的游戏
- 房内续局：全员投票通过后在同一房间开始下一局
- 在线更新：启动时通过 GitHub Releases 检查、下载并验证签名更新包
- 批量题库：支持固定 JSON 格式的合并导入与整库替换
...
构建 Windows NSIS 安装包。正式发布必须设置 Tauri 更新签名私钥：
...
13. 在开局后使用邀请码加入，并直接进入当前游戏。
14. 完成全员续局投票，并进入同一房间的下一局。
15. 使用 JSON 文件批量合并或替换私人题库。
16. 从 GitHub Releases 检查签名更新包并完成安装重启。
